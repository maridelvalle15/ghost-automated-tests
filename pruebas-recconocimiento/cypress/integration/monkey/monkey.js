...
describe( `${appName} under monkeys`, function() {
    ...
    it(`visits ${appName} and survives monkeys`, function() {
        ...
        if(pcg === 100){
            ...
            //login first
            cy.get('input[name="identification"]').then($inputs => {
                var inputName = $inputs.get(0);
                if(!Cypress.dom.isHidden(inputName)) {
                    cy.wrap(inputName).type("marisela.delvalle93@gmail.com");
                }
                cy.wait(1000);
            });
            cy.get('input[name="password"]').then($inputs => {
                var inputPassword = $inputs.get(0);
                if(!Cypress.dom.isHidden(inputPassword)) {
                    cy.wrap(inputPassword).type("12345678");
                }
                cy.wait(1000);
            });
            cy.get('button.login').then($buttons => {
                var loginButton = $buttons.get(0);
                if(!Cypress.dom.isHidden(loginButton)) {
